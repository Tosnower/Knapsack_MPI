...
//        c = atoi(argv[2]);        /* Capacity of knapsack */
    ...
    if (rank == 0) {
        ...
        for (int k = 1; k < size; k++) {
            ...
            MPI_Send(v, n, MPI_INT, k, 1, MPI_COMM_WORLD);
            MPI_Send(w, n, MPI_INT, k, 2, MPI_COMM_WORLD);
        }
      //printf("start1 %d\n",rank);
    }
    else {
        printf("zixiancheng %d \n",rank);
        MPI_Recv(&n, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        printf("zixiancheng2 %d \n",rank);
        v = (int*) malloc(sizeof(int) * n);
        w = (int*) malloc(sizeof(int) * n);
        MPI_Recv(v, n, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        printf("zixiancheng3 %d \n",rank);
        MPI_Recv(w, n, MPI_INT, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        printf("zixiancheng4 %d \n",rank);
    }
